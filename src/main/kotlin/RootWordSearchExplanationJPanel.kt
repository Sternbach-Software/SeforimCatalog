...
class RootWordSearchExplanationJPanel(private val searchableTableJPanel: SearchableTableJPanel) : JPanel() {
    ...
    private fun initComponents() {
        ...
        jLabel5!!.text =
            "<html><body style='width: %1spx'>Root word (שרש) search determines matching entries by comparing the root words contained in the search phrase with the root words in each catalog entry. Among other things, this removes the need to be concerned with whether a word is spelled malei or chaseir, contains prefixes (e.g. -ב-,מ-,ש...) or suffixes (e.g. ותם, -יהם-...), is masculine or feminine, plural or singular, etc. For example, entering \"תורתו\" will show results for \"ומתורתינו\", \"תורותיכם\", etc.<br><br><br>Only Hebrew is currently supported. Nouns are preserved in their full form (e.g. <font size=5> וּמִמַּאַמְרֵיהֶם</font> is reduced to <font size=5>מַאֲמָר</font> and not אמר), whereas verbs and adjectives are reduced to their root word. However, although the search engine may have a general understanding of dikduk (on occasion it may make a mistake - this may improve in the future), that does not make it a Talmid Chacham. For example, מאמרי can be read as <font size=5> מַאֲמָרֵי </font>or <font size=5>מֵאִמְרֵי</font>. The first reduces to מאמר and the second reduces to אמר, but without an understanding of the meaning and the context, not even a human would know how to properly punctuate it and therefore determine its shoresh. Note that if a word is not recognized as a valid hebrew word, it will not be added to the list of shorashim.</body>"
        setShorashim(listOf())
